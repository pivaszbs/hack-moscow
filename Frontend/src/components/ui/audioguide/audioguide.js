...
const Audioguide = (props) => {
    ...
    return <div className='audio-card'>
        ...
        <div className={clsx('audioguide', className)}>
            <Icon className='audioguide-icon' isPlaying={isPlaying} callback={setIsPlaying}/>
            <AudioBar timeOffset={timeOffset} maxTime={maxTime} isPlaying={isPlaying} callback={setTimeOffset}/>
        </div>
    </div>
};
...
const Icon = ({className, isPlaying, callback}) => {
    ...
    return <img className={clsx('icon', className)} src={iconSource} onClick={clickHandle}
                alt='icon'/>
};

const AudioBar = ({className, maxTime, timeOffset = 0, isPlaying, callback}) => {
    ...
    return <div className={clsx("slidecontainer", className)}>
        <PrettoSlider
